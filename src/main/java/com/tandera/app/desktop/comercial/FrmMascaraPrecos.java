...
public class FrmMascaraPrecos extends FormularioConsulta {
	...
	private void excluirItem() {
		MascaraPreco mascara = (MascaraPreco) tabela.getLinhaSelecionada();
		if (mascara == null) {
			SSMensagem.avisa("Selecione um item da lista");
			return;
		}

		if (SSMensagem.confirma("Confirma exclusão do Registro (" + mascara.getId() + "-"
				+ mascara.getMascara() + ")?")) {
			this.acao = Constantes.ACAO_EXCLUSAO;
			dao.deleteItemCategoria(mascara.getId());
